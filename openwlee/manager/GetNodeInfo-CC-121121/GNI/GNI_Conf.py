...
# -*- coding:gbk -*-
import os
from _modules_._ini_ import _iniCFG
...
confopts={
    ...
    'ConDat':{
        ...
        'CDDefinationFile':{
            'Value':'',
            'Type':'DTRING',
            'default':'./GNI/GNIConfLog/ConDatDef.ini',
    ...
    'LogConf':{
        'LogConfFile':{
            'Value':'',
            'Type':'STRING',
            'default':'./GNI//GNIConfLog/logging.conf',
...
confPath=os.getcwd()+'/GNI/GNIConfLog/GNI.conf'#os.getcwd()+'/config.ini'
...
def get_conf_opts(m_confopts=confopts,m_confPath=confPath):
    __GetConfOpts(m_confopts,m_confPath)


#Get conf value now
#get_conf_opts(confopts,confPath)
